...
#include "../../../modules/task_2/boganov_s_shell_batcher/shell_batcher.h"
...
std::vector<int> SortShellOMP(const std::vector<int>& vec, int number_of_parts) {
    ...
    return evenOddhMergeBatcherOMP(temp, number_of_parts);
}
...
std::vector<int> evenOddhMergeBatcherOMP(const std::vector<std::vector<int>>& vec, int number_of_parts) {
    ...
    while (k != 1) {
        ...
        #pragma omp parallel for private(evenTemp, oddTemp)
        for (p = 0; p < size; p++) {
            evenTemp = evenOrOddMerge(temp_pair[p].first, temp_pair[p].second, mergeType::even);
            oddTemp = evenOrOddMerge(temp_pair[p].first, temp_pair[p].second, mergeType::odd);
            temp2[p] = mergeBatcher(evenTemp, oddTemp);
